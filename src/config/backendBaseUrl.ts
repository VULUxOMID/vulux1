...
function readExpoDevHost(): string | undefined {
  ...
  try {
    ...
    const parsedCandidate = new URL(
      candidateUrl.replace(/^exp:/, 'http:').replace(/^exps:/, 'https:'),
    );
    return isLoopbackHost(parsedCandidate.hostname)
      ? undefined
      : normalize(parsedCandidate.hostname);
  } catch {
    return undefined;
  }
}
...
export function getConfiguredBackendBaseUrl(): string {
  return firstResolvedHttpUrl([
    process.env.EXPO_PUBLIC_EDGE_API_BASE_URL,
...
export function getConfiguredAdminApiBaseUrl(): string {
  return firstResolvedHttpUrl([
    ...
    process.env.EXPO_PUBLIC_LEGACY_API_BASE_URL,
...
export function getConfiguredUploadSignerBaseUrl(): string {
  return firstResolvedHttpUrl([
    process.env.EXPO_PUBLIC_UPLOAD_SIGNER_URL,
    process.env.EXPO_PUBLIC_EDGE_API_BASE_URL,
    process.env.EXPO_PUBLIC_BACKEND_API_BASE_URL,
  ]);
}
