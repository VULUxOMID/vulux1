...
export async function publishLiveInvite(input: LiveInviteInput): Promise<void> {
  ...
  await sendGlobalEvent(makeEventId('live-invite'), liveId, {
    ...
    targetUserId,
    inviterUserId,
    source: input.source === 'request' ? 'request' : 'host',
    createdAt: Date.now(),
  });
}
...
export async function publishLiveHostRequestResponse(
  ...
  const hostUserId = getSpacetimeAuthSnapshot().userId;
  if (!liveId || !requesterUserId || !hostUserId) {
    throw new Error('A live id, requester user, and signed-in host are required.');
  }
...
export async function publishLiveInviteResponse(input: LiveInviteResponseInput): Promise<void> {
  ...
  const targetUserId = normalizeString(input.targetUserId) ?? responderUserId;
  if (!liveId || !targetUserId || !responderUserId) {
